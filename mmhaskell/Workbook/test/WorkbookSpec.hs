...
testEvens i input expected =
  testCase ("Evens " ++ show i) $
...
testAddWhenMod3Is2 i input expected =
  testCase ("AddWhenMod3Is2 " ++ show i) $
    addWhenMod3Is2 input @?= expected

addWhenMod3Is2Tests :: TestTree
addWhenMod3Is2Tests =
  testGroup
    "AddWhenMod3Is2 Tests"
...
testReverse i input expected =
  testCase ("Reverse " ++ show i) $
...
testReverseAccum i input expected =
  testCase ("ReverseAccum " ++ show i) $
...
testSpecialMultiples i input expected =
  testCase ("SpecialMultiples " ++ show i) $
    specialMultiples input @?= expected

specialMultiplesTests :: TestTree
specialMultiplesTests =
  testGroup
    "SpecialMultiples Tests"
...
testManyStrings i (inputInts, inputStrs) expected =
  testCase ("ManyStrings " ++ show i) $
...
testAddPairs i input expected =
  testCase ("AddPairs " ++ show i) $
...
testListToMap i input expected =
  testCase ("ListToMap " ++ show i) $
...
testSumWithParity i input expected =
  testCase ("SumWithParity " ++ show i) $
...
testJumpingStairs i (inputInts, inputTuples) expected =
  testCase ("JumpingStairs " ++ show i) $
...
jumpingStairsTests =
  testGroup
    ...
    , testJumpingStairs
        3
        ([6, 4, 10, 3], [("First", 5), ("Second", 5), ("Third", 4), ("Fourth", 5)])
